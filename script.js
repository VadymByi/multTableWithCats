...
// Объект с таблицей умножения от 1 до 10(отдельно равенство, отдельно ответ)
...
// Функция  загрузки веселого или грустного котика
function showCat(isHappy) {
  ...
  fetch(url, {
    ...
    .catch((error) => console.error("Ошибка при получении котэ", error));
}

// Объект с картинками за неправильный ответ
// Запускаем тренировку кнопкой
start.addEventListener("click", () => {
  getRandomQuestion();
});
// Выводим случайное равенство и запрашиваем у пользователя решение - окно ввода
...
// Получаем ввод и сравниваем его с правильным ответом
// Функция для проверки введенного пользователем ответа
...
// Обработка нажатия кнопки Ответ
submitButton.addEventListener("click", function () {
  checkAnswer();
});

// Если решение правильное - выводим в отдельное окно картинку из массива за правильный ответ,
//  если неправильное - из массива за неправильный ответ и показываем правильный ответ
// Повторяем снова
